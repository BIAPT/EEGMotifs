function [intensity,coherence,frequency,frequency_motifs] = calculate_motifs(matrix,type,thresh,num_motifs,is_binary)
%CALCULATE_MOTIFS : 
...
if(strcmp(type,'dpli') || strcmp(type,'nste'))
    ...
    %% Motif Caculations
    [intensity,coherence,frequency,frequency_motifs] = compute(matrix,num_motifs,is_binary);
    if(isnan(frequency))
...
function [I,Q,F,f] = compute(t_matrix,num_motifs,is_binary)
    I = NaN;
    Q = NaN;
    F = NaN;
    f = NaN;
    if(is_binary)
       [F,f] = compute_bin(t_matrix,num_motifs);
    else
       [I,Q,F] = compute_wei(t_matrix,num_motifs);
    end
end
